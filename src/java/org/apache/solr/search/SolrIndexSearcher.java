...
/**
 * SolrIndexSearcher adds schema awareness and caching functionality over the
...
public class SolrIndexSearcher extends IndexSearcher implements SolrInfoMBean {
	private static Logger log = LoggerFactory
			.getLogger(SolrIndexSearcher.class);
	...
	private static final HashMap<String, SolrCache> noGenericCaches = new HashMap<String, SolrCache>(
			0);
	...
	public SolrIndexSearcher(SolrCore core, IndexSchema schema, String name,
		...
		this(core, schema, name, core.getIndexReaderFactory().newReader(
				core.getDirectoryFactory().open(path), false), true,
				enableCache);
	}
	...
	public SolrIndexSearcher(SolrCore core, IndexSchema schema, String name,
		...
		this(core, schema, name, core.getIndexReaderFactory().newReader(
				directory, false), true, enableCache);
	}

	/** Creates a searcher searching the index in the provided directory. */
	public SolrIndexSearcher(SolrCore core, IndexSchema schema, String name,
			Directory directory, boolean readOnly, boolean enableCache)
			throws IOException {
		this(core, schema, name, core.getIndexReaderFactory().newReader(
				directory, readOnly), true, enableCache);
	}
	...
	public SolrIndexSearcher(SolrCore core, IndexSchema schema, String name,
		...
		if (cachingEnabled) {
			...
			filterCache = solrConfig.filterCacheConfig == null ? null
			...
			if (solrConfig.userCacheConfigs == null) {
				...
				cacheMap = new HashMap<String, SolrCache>(
						solrConfig.userCacheConfigs.length);
		...
		optimizer = solrConfig.filtOptEnabled ? new LuceneQueryOptimizer(
				solrConfig.filtOptCacheSize, solrConfig.filtOptThreshold)
				: null;
	...
	public void close() throws IOException {
		...
		// super.close();
		// can't use super.close() since it just calls reader.close() and that
		...
		if (storedHighlightFieldNames == null) {
			...
			for (String fieldName : fieldNames) {
				try {
					...
					if (field.stored()
							&& ((field.getType() instanceof org.apache.solr.schema.TextField) || (field
									.getType() instanceof org.apache.solr.schema.StrField))) {
						storedHighlightFieldNames.add(fieldName);
					}
				} catch (RuntimeException e) { // getField() throws a
												// SolrException, but it arrives
												// as a RuntimeException
					log.warn("Field \"" + fieldName
							+ "\" found in index, but not defined in schema.");
				}
			}
		}
	...
	//
	// Set default regenerators on filter and query caches if they don't have
	// any
	//
			...
			solrConfig.queryResultCacheConfig
					.setRegenerator(new CacheRegenerator() {
						public boolean regenerateItem(
							...
							int nDocs = 1;
							// request 1 doc and let caching round up to the
							// next window size...
							// unless the window size is <=1, in which case we
							// will pick
							// the minimum of the number of documents requested
							// last time and
							...
							qc.setQuery(key.query).setFilterList(key.filters)
									.setSort(key.sort).setLen(nDocs)
									.setSupersetMaxDoc(nDocs).setFlags(flags);
	 ...
	 * @see org.apache.solr.search.SolrIndexSearcher#SolrIndexSearcher(org.apache.solr.core.SolrCore,
	 ...
	 * 
	 * If no cache is present: - use NO_LOAD instead of LAZY_LOAD - use
	 * LOAD_AND_BREAK if a single field is begin retrieved
	 */

	/**
	 * FieldSelector which loads the specified fields, and load all other field
	 * lazily.
	 */
	...
	/**
	 * Retrieve a {@link Document} using a
	 * {@link org.apache.lucene.document.FieldSelector} This method does not
	 * currently use the Solr document cache.
	 * 
	 ...
	 * Retrieve the {@link Document} instance corresponding to the document id.
	 * 
	 * Note: The document will have all fields accessable, but if a field filter
	 * is provided, only the provided fields will be loaded (the remainder will
	 * be available lazily).
	 */
	 ...
	 */
	public void readDocs(Document[] docs, DocList ids, Set<String> fields)
			throws IOException {
	 ...
	 * Returns -1 if no document was found. This method is primarily intended
	 * for clients that want to fetch documents using a unique identifier."
	 * 
	 * @param t
	...
	/**
	 * Compute and cache the DocSet that matches a query. The normal usage is
	 * expected to be cacheDocSet(myQuery, null,false) meaning that Solr will
	 * determine if the Query warrants caching, and if so, will compute the
	 * DocSet that matches the Query and cache it. If the answer to the query is
	 * already cached, nothing further will be done.
	 * <p>
	 ...
	 * @param mustCache
	 *            if true, a best effort will be made to cache this entry. if
	 *            false, heuristics may be used to determine if it should be
	 *            cached.
	 */
	public void cacheDocSet(Query query, DocSet optionalAnswer,
			boolean mustCache) throws IOException {
		// Even if the cache is null, still compute the DocSet as it may serve
		// to warm the Lucene
		// or OS disk cache.
	 ...
	 * cache-aware and attempts to retrieve the answer from the cache if
	 * possible. If the answer was not cached, it may have been inserted into
	 * the cache as a result of this call. This method can handle negative
	 * queries.
	 * <p>
	...
	public DocSet getDocSet(Query query) throws IOException {
		...
		if (filterCache != null) {
			...
			if (absAnswer != null) {
				if (positive)
					...
					return getPositiveDocSet(matchAllDocsQuery).andNot(
							absAnswer);
			}
		}
		...
		DocSet answer = positive ? absAnswer : getPositiveDocSet(
				matchAllDocsQuery).andNot(absAnswer);
	...
	public DocSet getDocSet(List<Query> queries) throws IOException {
		if (queries == null)
			return null;
		if (queries.size() == 1)
			return getDocSet(queries.get(0));
		DocSet answer = null;
	 ...
	 * This method is cache-aware and attempts to retrieve the answer from the
	 * cache if possible. If the answer was not cached, it may have been
	 * inserted into the cache as a result of this call.
	 * <p>
	 * 
	 * @param query
	 * @param filter
	...
	public DocSet getDocSet(Query query, DocSet filter) throws IOException {
		...
		return positive ? getDocSetNC(absQ, filter) : filter
				.andNot(getPositiveDocSet(absQ));
	}

	/**
	 * Converts a filter into a DocSet. This method is not cache-aware and no
	 * caches are checked.
	 */
	 ...
	 * <code>filter</code> and sorted by <code>sort</code>.
	 * <p>
	 ...
	 * @throws IOException
	 */
	public DocList getDocList(Query query, Query filter, Sort lsort,
			int offset, int len) throws IOException {
		QueryCommand qc = new QueryCommand();
		qc.setQuery(query).setFilterList(filter).setSort(lsort)
				.setOffset(offset).setLen(len);
		QueryResult qr = new QueryResult();
		search(qr, qc);
	 ...
	 * @throws IOException
	 */
	public DocList getDocList(Query query, List<Query> filterList, Sort lsort,
			int offset, int len, int flags) throws IOException {
		QueryCommand qc = new QueryCommand();
		qc.setQuery(query).setFilterList(filterList).setSort(lsort)
				.setOffset(offset).setLen(len).setFlags(flags);
		QueryResult qr = new QueryResult();
		search(qr, qc);
		return qr.getDocList();
	}
	 ...
	 * event of a timeout, the cache is not populated.
	 */
	private void getDocListC(QueryResult qr, QueryCommand cmd)
			throws IOException {
		// old parameters: DocListAndSet out, Query query, List<Query>
		// filterList, DocSet filter, Sort lsort, int offset, int len, int
		// flags, long timeAllowed, NamedList<Object> responseHeader
		...
		try { 
			cmd.getQuery().extractTerms(terms);
		} catch (Exception ex) {
			terms = null;
		}  
	...
	private void getDocListNC(QueryResult qr, QueryCommand cmd)
		...
		// Parameters:
		// cmd.getQuery(),theFilt,cmd.getSort(),0,supersetMaxDoc,cmd.getFlags(),cmd.getTimeAllowed(),responseHeader);
		// Query query, DocSet filter, Sort lsort, int offset, int len, int
		// flags, long timeAllowed, NamedList<Object> responseHeader

		// in case of filter is by solbaseIndexedInt field, we don't want to create DocSet, we can skip all of this
		//DocSet filter = cmd.getFilter() != null ? cmd.getFilter()
		//		: getDocSet(cmd.getFilterList());
		
		// hmm, i'm not seeing getFilter returning obj at all, maybe it's deprecated?
		// so then I need to create embedded doc set that's going to support multiple embeddedfieldfilter
		DocSet filter = null;
		
		if(cmd.getFilter() != null){
			filter = cmd.getFilter();
		} else if(cmd.getFilterList() != null){
			List<Query> filterList = cmd.getFilterList();
			
			filter = new EmbeddedDocSet();
			
			for (Query q : filterList) {
				// if range query get upper and lower bound
				if (q instanceof TermRangeQuery) {
					TermRangeQuery trq = (TermRangeQuery) q;
					SchemaField sf = schema.getField(trq.getField());
					if (sf.getType() instanceof EmbeddedIndexedIntField) {
						EmbeddedIndexedIntField siif = (EmbeddedIndexedIntField) sf.getType();
						int lowerBound, upperBound;

						if (trq.getUpperTerm() == null) {
							upperBound = Integer.MAX_VALUE;
						} else {
							upperBound = new Integer(trq.getUpperTerm());
						}
						if (trq.getLowerTerm() == null) {
							lowerBound = 0;
						} else {
							lowerBound = new Integer(trq.getLowerTerm());
						}
						((EmbeddedDocSet)filter).addFilter(lowerBound, upperBound, siif.getFieldNumber());
					} else {
					...
					SchemaField sf = schema.getField(tq.getField());
					if (sf.getType() instanceof EmbeddedIndexedIntField) {
						EmbeddedIndexedIntField siif = (EmbeddedIndexedIntField) sf.getType();
						// need to convert string to interger mapping here
						String term = tq.getTerm().toString();
						((EmbeddedDocSet)filter).addFilter(siif.getStringToInt(term), siif.getFieldNumber());
					} else {
						throw new UnsupportedOperationException();
					}
				}
			...
			// nullify EmbeddedDocSet in case it didn't have any of embedded filter created
			// this will only happen if unsupported operation exception is thrown though.
			if(((EmbeddedDocSet) filter).isEmptyFilter()){
				filter = null;
			}
		}
		...
		final Filter luceneFilter = filter == null ? null : filter
				.getTopFilter();
		...
		if (lastDocRequested <= 0) {
			...
			if (!needScores) {
				collector = new Collector() {
					...
					public void collect(int doc, int[] sorts)
							throws IOException {
						numHits[0]++;
					}
			...
			sorts = new int[nDocsReturned][Scorer.numSort];
			...
			if (cmd.getSort() == null) {
				...
				topCollector = TopFieldCollector.create(cmd.getSort(), len,
		...
		if (sliceLen < 0)
			sliceLen = 0;
		qr.setDocList(new DocSlice(0, sliceLen, ids, scores, totalHits,
				maxScore, sorts));
	}
	...
	private DocSet getDocListAndSetNC(QueryResult qr, QueryCommand cmd)
			throws IOException {
		int len = cmd.getSupersetMaxDoc();
		DocSet filter = cmd.getFilter() != null ? cmd.getFilter()
				: getDocSet(cmd.getFilterList());
		...
		if (lastDocRequested <= 0) {
			...
			if (!needScores) {
				...
				collector = setCollector = new DocSetDelegateCollector(
						smallSetSize, maxDoc, new Collector() {
							Scorer scorer;
	...
	/**
	 * Returns documents matching both <code>query</code> and
	 * <code>filter</code> and sorted by <code>sort</code>. FUTURE: The returned
	 * DocList may be retrieved from a cache.
	 * 
	 * @param query
	 * @param filter
	 *            may be null
	 ...
	 * @throws IOException
	 */
	public DocList getDocList(Query query, DocSet filter, Sort lsort,
			int offset, int len) throws IOException {
		QueryCommand qc = new QueryCommand();
		qc.setQuery(query).setFilter(filter).setSort(lsort).setOffset(offset)
				.setLen(len);
		QueryResult qr = new QueryResult();
		search(qr, qc);
		return qr.getDocList();
	...
	/**
	 * Returns documents matching both <code>query</code> and
	 * <code>filter</code> and sorted by <code>sort</code>. Also returns the
	 * complete set of documents matching <code>query</code> and
	 * <code>filter</code> (regardless of <code>offset</code> and
	 * <code>len</code>).
	 * <p>
	 * This method is cache aware and may retrieve <code>filter</code> from the
	 * cache or make an insertion into the cache as a result of this call.
	 ...
	 * @throws IOException
	 */
	public DocListAndSet getDocListAndSet(Query query, Query filter,
			Sort lsort, int offset, int len) throws IOException {
		QueryCommand qc = new QueryCommand();
		qc.setQuery(query).setFilterList(filter).setSort(lsort)
				.setOffset(offset).setLen(len).setNeedDocSet(true);
		QueryResult qr = new QueryResult();
		search(qr, qc);
	 ...
	 * Returns documents matching both <code>query</code> and
	 * <code>filter</code> and sorted by <code>sort</code>. Also returns the
	 * compete set of documents matching <code>query</code> and
	 * <code>filter</code> (regardless of <code>offset</code> and
	 * <code>len</code>).
	 * <p>
	 * This method is cache aware and may retrieve <code>filter</code> from the
	...
	/**
	 * Returns documents matching both <code>query</code> and the intersection
	 * of <code>filterList</code>, sorted by <code>sort</code>. Also returns the
	 * compete set of documents matching <code>query</code> and
	 * <code>filter</code> (regardless of <code>offset</code> and
	 * <code>len</code>).
	 * <p>
	 ...
	 * @throws IOException
	 */
	public DocListAndSet getDocListAndSet(Query query, DocSet filter,
			Sort lsort, int offset, int len) throws IOException {
		QueryCommand qc = new QueryCommand();
		qc.setQuery(query).setFilter(filter).setSort(lsort).setOffset(offset)
				.setLen(len).setNeedDocSet(true);
		QueryResult qr = new QueryResult();
		search(qr, qc);
		return qr.getDocListAndSet();
	}
	 ...
	 * <code>len</code>).
	 * <p>
	 * This method is cache aware and may make an insertion into the cache as a
	 * result of this call.
	 * <p>
	 * FUTURE: The returned DocList may be retrieved from a cache.
	 * <p>
	...
	public DocListAndSet getDocListAndSet(Query query, DocSet filter,
		...
		QueryCommand qc = new QueryCommand();
		qc.setQuery(query).setFilter(filter).setSort(lsort).setOffset(offset)
				.setLen(len).setFlags(flags).setNeedDocSet(true);
		QueryResult qr = new QueryResult();
		search(qr, qc);
		return qr.getDocListAndSet();
	}

	protected DocList sortDocSet(DocSet set, Sort sort, int nDocs)
			throws IOException {
		// bit of a hack to tell if a set is sorted - do it better in the
		// futute.
		...
		while (iter.hasNext()) {
			...
			while (doc >= end) {
				...
				// we should never need to set the scorer given the settings for
				// the collector
			}
			topCollector.collect(doc - base, null);
		}
		...
		int[][] sorts = new int[nDocsReturned][Scorer.numSort];
		...
		return new DocSlice(0, nDocsReturned, ids, null, topDocs.totalHits,
				0.0f, sorts);
	}

	/**
	 * Returns the number of documents that match both <code>a</code> and
	 * <code>b</code>.
	 * <p>
	 ...
	 * @param a
	 * @param b
	 * @return the numer of documents in the intersection between <code>a</code>
	 *         and <code>b</code>.
	 * @throws IOException
	 */
	public int numDocs(Query a, DocSet b) throws IOException {
		...
		return a == absQ ? b.intersectionSize(positiveA) : b
				.andNotSize(positiveA);
	}
	...
	public int numDocs(Query a, Query b) throws IOException {
		...
		// we use the last form since the intermediate DocSet should normally be
		// smaller.
		return all.andNotSize(positiveA.union(positiveB));
	}

	/**
	 * Takes a list of docs (the doc ids actually), and returns an array of
	 * Documents containing all of the stored fields.
	 */
	...
	/**
	 * Warm this searcher based on an old one (primarily for auto-cache
	 * warming).
	 */
	...
	private int mapStringFieldToInt(String term, String fieldName){
		return -1;
	}
}
