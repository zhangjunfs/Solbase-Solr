...
public class DocSlice extends DocSetBase implements DocList {
  public final int offset;    // starting position of the docs (zero based)
  public final int len;       // number of positions used in arrays
   ...
   */
  public DocSlice(int offset, int len, int[] docs, float[] scores, int matches, float maxScore, int[][] sorts) {
    this.offset=offset;
    this.len=len;
    this.docs=docs;
    this.scores=scores;
    ...
    this.sorts = null;
    if (sorts != null) {
    	this.sorts = new int[sorts.length][Scorer.numSort];
	    for (int i = 0; sorts != null && i < sorts.length; i++) {
	    	DocIdSetIterator.copySorts(sorts[i], this.sorts[i]);
	    }
    }
  }
  ...
  public DocList subset(int offset, int len) {
    ...
    return new DocSlice(offset, realLen, docs, scores, matches, maxScore, sorts);
  }
