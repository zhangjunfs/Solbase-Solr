   ...
   */
	public static Document toDocument(SolrInputDocument doc, IndexSchema schema) {
		...
		for (SolrInputField field : doc) {
			...
			for (Object v : field) {
				...
				if (sfield != null && sfield.getType() instanceof BinaryField) {
					...
					if (sfield != null) {
						if (sfield.getType() instanceof EmbeddedIndexedIntField) {
							...
							EmbeddedSortField sf = new EmbeddedSortField(name,
																		val, 
																		Field.Store.YES,
																		Field.Index.NO,
																		eiif.getFieldNumber());
				...
				for (CopyField cf : copyFields) {
					...
					if (!destinationField.multiValued()
							&& out.get(destinationField.getName()) != null) {
						throw new SolrException(
								SolrException.ErrorCode.BAD_REQUEST,
					...
					if (isBinaryField) {
						if (destinationField.getType() instanceof BinaryField) {
							BinaryField binaryField = (BinaryField) destinationField
									.getType();
							f = binaryField.createField(destinationField, v,
									boost);
						}
					} else {

						if (destinationField.getType() instanceof EmbeddedIndexedIntField) {
							EmbeddedIndexedIntField eiif = (EmbeddedIndexedIntField)destinationField.getType();
							
							EmbeddedSortField sf = new EmbeddedSortField(name,
																		cf.getLimitedValue(val), 
																		Field.Store.YES,
																		Field.Index.NO,
																		eiif.getFieldNumber());
							out.add(sf);
						} else {
							f = destinationField.createField(
								cf.getLimitedValue(val), boost);
						}
					}
					if (f != null) { // null fields are not added
						out.add(f);
			...
			/* TODO: do we want to define field in schema for edit?
			 * some fields on solr document is only used between solbase and update client
			if (!used && hasField) {
		...
		/*
		 * TODO: assuming, docs already got all required fields
		 * this is because EmbeddedSortField class cast issue in Document getField()
		...
		for (SchemaField field : schema.getRequiredFields()) {
			if (out.getField(field.getName()) == null) {
				if (field.getDefaultValue() != null) {
					...
					throw new SolrException(
							SolrException.ErrorCode.BAD_REQUEST, msg);
				}
			}
		}
		*/
